...
class BukuBesarApi 
{
    ...
    {
        $userId = 0;
       $items = MstAkunModel::where('created_by', $userId)
        ->orderBy('kode_akun')
...
public function storeSaldoAwal(Request $r) //terpakai
{
    ...
    if (count($nominal) === 0) {
        return response()->json(['ok'=>false,'message'=>'Nominal wajib diisi'], 422);
    }
    ...
    try {
        DB::transaction(function () use ($mstId, $subIds, $nominal, $tanggal, $count, $total, $userId) {
            ...
            if (!$mst) {
                throw new \Exception("Akun tidak ditemukan (mst_akun_id=$mstId, created_by=$userId).");
            }
